...
function initHomeSlider(scope) {
  ...
  // Инициализация: все work_slide_img и work_slide_info прозрачные, кроме первого
  ...
  // Инициализация: первый слайд активен, divider в первом thumb
  slides[0].style.zIndex = zIndexCounter++;
  slides[0].style.clipPath = "inset(0% 0% 0% 0%)";
  thumbs[0].appendChild(divider);

  // Функция для блокировки всех thumbs
  function disableAllThumbs() {
    console.log('=== DISABLE ALL THUMBS CALLED ===');
    console.log('isClicked was:', isClicked);
    console.log('activeIndex:', activeIndex);
    console.log('thumbs count:', thumbs.length);
    console.log('slides count:', slides.length);
    console.log('thumbs array:', thumbs);
    console.log('slides array:', slides);
    
    isClicked = true;
    
    // Отключаем pointer-events для всех ссылок внутри других work_thumb_wrap
    thumbs.forEach((thumb, index) => {
      if (index !== activeIndex) {
        // Ищем ссылки в разных возможных местах
        const workThumbLinks = thumb.querySelectorAll('.work_thumb a');
        const allLinks = thumb.querySelectorAll('a');
        
        // Отключаем pointer-events для ссылок в .work_thumb
        workThumbLinks.forEach((link) => {
          link.style.pointerEvents = 'none';
        });
        
        // Также отключаем pointer-events для всех ссылок в thumb
    ...
    // Делаем все остальные work_slide прозрачными и скрываем их
    slides.forEach((slide, index) => {
      console.log(`Processing slide ${index}, activeIndex: ${activeIndex}`);
      if (index !== activeIndex) {
        gsap.to(slide, {
          opacity: 0,
          duration: 0.1,
          ...
          onComplete: () => {
            ...
            console.log(`Slide ${index} display set to none`);
          }
        });
        console.log(`Slide ${index} made transparent and hidden`);
      } else {
        console.log(`Skipping active slide ${index}`);
      }
    });
    
    console.log('=== DISABLE ALL THUMBS COMPLETED ===');
    barbaLog('All work_thumb_wrap elements disabled after click');
  }

  thumbs.forEach((thumb, i) => {
    // Обработчик клика на thumb
    thumb.addEventListener('mousedown', (e) => {
      console.log(`=== MOUSEDOWN ON THUMB ${i} ===`);
    ...
    thumb.addEventListener('mouseenter', () => {
      ...
      // Убеждаемся, что слайд видим перед анимацией
      ...
      // work_slide_img: сразу opacity 1, затем scale анимация
      const img = slide.querySelector('.work_slide_img');
      if (img) {
      ...
      // work_slide_info: анимируем появление
      const info = slide.querySelector('.work_slide_info');
      if (info) {
        // Убеждаемся, что элемент видим и анимируем появление
        gsap.fromTo(
          info,
          { opacity: 0 },
          {
      ...
      // work_slide_img и work_slide_info в предыдущем активном слайде делаем прозрачными
    ...
    thumb.addEventListener('mouseleave', () => {
      // После клика блокируем mouseleave для активного слайда
      if (isClicked && activeIndex === i) return;
      
      setTimeout(() => {
        const isAnyHovered = thumbs.some(t => t.matches(':hover'));
        
        // Если этот thumb был активным, но мы ушли с него
        if (activeIndex === i) {
          if (!isAnyHovered) {
            // Если нет других hover, возвращаемся к первому слайду
            const targetIndex = 0;
            const targetSlide = slides[targetIndex];
            
            // Убеждаемся, что слайд видим
            ...
            // Анимируем переход к целевому слайду
            gsap.to(targetSlide, {
              clipPath: "inset(0% 0% 0% 0%)",
              duration: 1.4,
              ease: "expo.out",
            ...
            // Показываем img и info целевого слайда
            ...
            // Скрываем img и info текущего активного слайда только если это не тот же слайд
            if (i !== targetIndex) {
            ...
            // Если есть другой hover, скрываем текущий активный слайд через clipPath
            // mouseenter другого thumb обработает показ нового слайда
            // Уменьшаем z-index, чтобы скрытый слайд не перекрывал новые
  ...
  // Добавляем обработчики кликов для всех slides
  slides.forEach((slide, index) => {
    slide.addEventListener('mousedown', (e) => {
      console.log(`=== MOUSEDOWN ON SLIDE ${index} ===`);
      console.log('isClicked:', isClicked);
      e.preventDefault();
      e.stopPropagation();
      disableAllThumbs();
...
function initNav(scope) {
  ...
  barbaLog('Nav initialized', {
    hasNav: !!nav,
    hasTrigger: !!trigger,
    hasAboutTabs: aboutTabs.length
  });
}
...
function initArchiveThumb(scope) {
  ...
  barbaLog('Archive thumb initialized', {
    thumbsCount: archiveThumbs.length,
    textElsCount: archiveTextEls.length
  });
}
